...
def initialize_modules(
    ...
    def _safe_log_info(message: str):
        try:
    ...
    if init_tts:
        try:
            ...
            _safe_log_info(f"初始化组件: tts成功 {select_tts_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: tts失败 {e}")
    ...
    if init_llm:
        try:
            ...
            _safe_log_info(f"初始化组件: llm成功 {select_llm_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: llm失败 {e}")
    ...
    if init_intent:
        try:
            ...
            _safe_log_info(f"初始化组件: intent成功 {select_intent_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: intent失败 {e}")
    ...
    if init_memory:
        try:
            ...
            _safe_log_info(f"初始化组件: memory成功 {select_memory_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: memory失败 {e}")
    ...
    if init_vad:
        try:
            ...
            _safe_log_info(f"初始化组件: vad成功 {select_vad_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: vad失败 {e}")
    ...
    if init_asr:
        try:
            ...
            _safe_log_info(f"初始化组件: asr成功 {select_asr_module}")
        except Exception as e:
            _safe_log_error(f"初始化组件: asr失败 {e}")
    return modules
